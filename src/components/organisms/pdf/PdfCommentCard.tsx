 ...
 * language) - binding it to its in-doc zone. The margin SNAPS and framer's `layout` slides the stack into its
 * new places via transforms, so the reflow rides the compositor instead of relaying out every card each frame.
 * It scrolls itself into view with a soft one-shot entrance (static under reduced motion). Chrome uses tokens;
 * the accent bar and color dot are the comment's hex.
 */
...
export function PdfCommentCard({ comment, isFocused, isEditing, onJump, onStartEdit, onChangeBody, onEndEdit, onDelete, onLinkActivate }: PdfCommentCardProps) {
   ...
   return (
      <motion.div
         ref={ref}
         // The focused card's margin snaps (no CSS tween); framer repositions the whole stack via transforms
         // so neighbours slide cleanly, instead of the margin animating pixel-by-pixel and relaying out every
         // card each frame. Position-only, so the box's own size change (border / content) never distorts.
         layout="position"
         transition={{ layout: { duration: reduceMotion ? 0 : 0.24, ease: 'easeOut' } }}
         className={cn(isFocused && 'my-3')}
      >
         <div
            // The lift / lean pops on transforms (cheap); the border thickens instantly - animating
            // border-width reads as jank.
            className={cn(
               'rounded-lg border border-border bg-card p-2.5 shadow-sm transition-[transform,box-shadow]',
               isFocused && '-translate-x-1.5 border-l-4 shadow-lg motion-safe:animate-[cotm-comment-pop_240ms_ease-out]',
            )}
