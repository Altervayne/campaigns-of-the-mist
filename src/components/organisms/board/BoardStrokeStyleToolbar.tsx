 ...
 * they route to the transform commit). Color / width preview live and commit on release (ONE undo step); a
 * brush / fill / flip click commits at once. Same frosted look + counter-scale as `BoardGroupToolbar`, with
 * the off-edge clamps the canvas measures.
 */
...
export function BoardStrokeStyleToolbar({ zoom, fold, onPreviewStyle, onCommitStyle, onFlip, clampDown = 0, clampX = 0, measureRef }: BoardStrokeStyleToolbarProps) {
      ...
      <div ref={measureRef} className="pointer-events-auto absolute left-0" style={{ bottom, transformOrigin: '0 100%', transform }}>
         ...
         <div className="pb-2">
            <div className="flex items-center gap-0.5 rounded-lg border border-border bg-popover/90 p-1 shadow-md backdrop-blur-sm">
               ...
               <FlipButton title={t('BoardView.flipHorizontal')} onClick={() => onFlip('x')}><FlipHorizontal className="h-4 w-4" /></FlipButton>
               <FlipButton title={t('BoardView.flipVertical')} onClick={() => onFlip('y')}><FlipVertical className="h-4 w-4" /></FlipButton>
            </div>
         </div>
      </div>
   );
...
function FlipButton({ title, onClick, children }: { title: string; onClick: () => void; children: ReactNode }) {
   return (
      <button
         type="button"
         title={title}
         ...
         onClick={onClick}
         className="flex size-6 shrink-0 items-center justify-center rounded text-popover-foreground hover:bg-muted cursor-pointer"
      >
         {children}
      </button>
   );
