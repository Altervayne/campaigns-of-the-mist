   ...
   return (
      <div ref={rowRef} data-tutorial="note-toolbar" className="flex items-center gap-0.5 border-b border-border bg-popover px-2 py-1.5">
         ...
         <button
            ...
            className={cn(
               'grid h-7 w-7 touch-target place-items-center rounded cursor-pointer',
...
function ToolbarButton({
   ...
   return (
      <button
         ...
         onMouseDown={(event) => event.preventDefault()}
         onClick={onClick}
         className="grid h-7 w-7 touch-target place-items-center rounded text-foreground hover:bg-muted cursor-pointer disabled:cursor-default disabled:opacity-60"
      >
         {children}
      </button>
   );
      ...
      <Popover open={open} onOpenChange={onOpenChange}>
         <PopoverTrigger asChild>
            <button
               ...
               onMouseDown={(event) => event.preventDefault()}
               className="grid h-7 w-7 touch-target place-items-center rounded text-foreground hover:bg-muted cursor-pointer"
            >
